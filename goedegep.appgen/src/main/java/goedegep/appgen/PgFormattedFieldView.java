...
public class PgFormattedFieldView extends FieldView {
  ...
  public PgFormattedFieldView(Element elem, PgFormattedTextField.FormatSpec formatSpec) {
    super(elem);
  ...
  public int viewToModel(float fx, float fy, Shape a, Bias[] bias) {
    ...
    int offset = Utilities.getTabbedTextOffset(contentBuff, getFontMetrics(),
                                               r.x, x, this, startOffset);
  ...
  protected void buildMapping(PgFormattedTextField.FormatSpec formatSpec) {
    ...
    if (formatSize != 0) {
      ...
      for (int i = 0; i < formatSize; i++) {
        if (maskChars[i] == PgFormattedTextField.FormatSpec.MARKER_CHAR) {
