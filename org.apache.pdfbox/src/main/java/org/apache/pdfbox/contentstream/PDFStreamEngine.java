...
public abstract class PDFStreamEngine
{
    ...
    public void showTextStrings(COSArray array) throws IOException
    {
        ...
        for (COSBase obj : array)
        {
            ...
            {
                LOG.severe(MessageFormat.format("Nested arrays are not allowed in an array for TJ operation: {0}", obj));
            }
            else
            {
                LOG.severe(MessageFormat.format("Unknown type {0} in array for TJ operation: {1}",
    ...
    protected void showText(byte[] string) throws IOException
    {
        ...
        if (font == null)
        {
            LOG.warning("No current font, will use default");
            font = getDefaultFont();
        }
    ...
    public void decreaseLevel()
    {
        ...
        if (level < 0)
        {
            LOG.severe(MessageFormat.format("level is {0}", level));
        }
    }
