...
final class DCTFilter extends Filter
{
        ...
        try (ImageInputStream iis = ImageIO.createImageInputStream(encoded))
        {
            ...
            if (raster.getNumBands() == 4)
            {
                ...
                catch (IIOException | NegativeArraySizeException e)
                {
                    ...
                    LOG.info("Couldn't read usíng getAdobeTransform() - using getAdobeTransformByBruteForce() as fallback" + e);
                ...
                switch (colorTransform)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        LOG.warning("There is no 4 channel YCbCr, using YCCK");
                        // fallthrough
                    case 2:
    ...
    private Raster readImageRaster(ImageReader reader, ImageReadParam irp) throws IOException
    {
        ...
        if ("3".equals(numChannels) || numChannels.isEmpty())
        {
            ...
            catch (IIOException e)
            {
                ...
                LOG.info("Couldn't read use read() for RGB image - using readRaster() as fallback" + e);
    ...
    private Integer getAdobeTransform(IIOMetadata metadata)
    {
        ...
        if (app14AdobeNodeList != null)
        {
            ...
            if (app14AdobeNodeListLength > 0)
            {
                ...
                {
                    LOG.warning("app14Adobe entry appears several times, using the last one");
                }
    ...
    private String getNumChannels(ImageReader reader)
    {
        ...
        {
            LOG.info("Couldn't read metadata - returning empty string" + e);
            return "";
        }
    }    
