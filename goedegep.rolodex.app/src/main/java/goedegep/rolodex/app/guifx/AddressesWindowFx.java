...
public class AddressesWindowFx extends JfxStage {
  ...
  public AddressesWindowFx(CustomizationFx customization, Rolodex rolodex) {
    ...
    addressesTable.addObjectSelectionListener(new ObjectSelectionListener<Address>() {

      @Override
      public void objectSelected(Address address) {
        referredByPanel.setObject(address);
        addressEditPanel.setAddress(address);
      }
...
class AddressEditPanel {
  ...
  private void createGUI() {
    ...
    gridPane.add(houseNumberExtensionTextField, 5, row);
    ...
    gridPane.add(poBoxTextField, 1, row);
    ...
    gridPane.add(postalCodeTextField, 1, row);
  ...
  private void handleNewCountryName() {
    if (countryTextField.isNonExistingCountyName()) {
     ...
     alert.showAndWait().filter(response -> response == ButtonType.YES).ifPresent(response -> {
        ...
        gridPane.add(countryTextField, 1, countryTextFieldRow);
      });
    }
  }
  ...
  private void handleNewCityName() {
    ...
    if (cityTextField.isNonExistingCityName()) {
      ...
      alert.showAndWait().filter(response -> response == ButtonType.YES).ifPresent(response -> {
        ...
        gridPane.add(cityTextField, 1, cityTextFieldRow);
      });
    }
...
class AddressesTableDescriptor extends EObjectTableDescriptor<Address> {
  ...
  private static Map<TableRowOperation, TableRowOperationDescriptor<Address>> rowOperations = new HashMap<>() {
    {
