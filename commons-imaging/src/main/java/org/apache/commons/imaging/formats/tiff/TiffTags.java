...
final class TiffTags {
    ...
    private static Map<Integer, Integer> countTags(final List<TagInfo> tagInfos) {
        final Map<Integer, Integer> map = new HashMap<>();

        for (final TagInfo tagInfo : tagInfos) {
            final Integer count = map.get(tagInfo.tag);
            if (count == null) {
                map.put(tagInfo.tag, 1);
            } else {
                map.put(tagInfo.tag, count + 1);
            }
        }
