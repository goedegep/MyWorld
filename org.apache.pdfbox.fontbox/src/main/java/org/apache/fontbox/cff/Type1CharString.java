...
public class Type1CharString
{
    ...
    private void handleType1Command(List<Number> numbers, CharStringCommand command)
    {
        ...
        if (type1KeyWord == null)
        {
            ...
            LOG.severe("Unknown charstring command in glyph {} of font {}" + glyphName + fontName);
            numbers.clear();
            return;
        }
        switch(type1KeyWord)
        ...
        case CALLSUBR:
            // indicates an invalid charstring
            LOG.severe("Unexpected charstring command: {} in glyph {} of font {}" + type1KeyWord +
                    glyphName + fontName);
            break;
        default:
    ...
    private void callothersubr(int num)
    {
        if (num == 0)
        {
            ...
            {
                LOG.severe("flex without moveTo in font {}, glyph {}, command {}" + fontName +
        ...
        {
            LOG.severe("Invalid callothersubr parameter: {}" + num);
        }
    }
    ...
    private void rlineTo(Number dx, Number dy)
    {
        ...
        {
            LOG.severe("rlineTo without initial moveTo in font {}, glyph {}" + fontName + glyphName);
            path.moveTo(x, y);
        }
        else
        {
            ...
            Number dx3, Number dy3)
    {
        ...
        if (path.getCurrentPoint() == null)
        {
            LOG.severe("rrcurveTo without initial moveTo in font {}, glyph {}" + fontName + glyphName);
            path.moveTo(x3, y3);
        }
        else
        {
    ...
    private void closeCharString1Path()
    {
        if (path.getCurrentPoint() == null)
        {
            LOG.severe("closepath without initial moveTo in font {}, glyph {}" + fontName + glyphName);
        }
        else
        {
            path.closePath();
    ...
    private void seac(Number asb, Number adx, Number ady, Number bchar, Number achar)
    {
        ...
        {
            LOG.severe(() -> "invalid seac character in glyph " + glyphName + " of font " + fontName + e);
        }
        // accent character
        ...
        try
        {
            ...
            if (path == accentPath)
            {
                ...
                LOG.severe("Path for {} and for accent {} are same, ignored" + baseName + accentName);
                return;
            }
