...
public class CmapSubtable implements CmapLookup
{
    ...
    void processSubtype8(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        if (numGlyphs == 0)
        {
            LOG.warning("subtable has no glyphs");
            return;
        }
        // -- Read all sub header
    ...
    void processSubtype12(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        for (long i = 0; i < nbGroups; ++i)
        {
            ...
            for (long j = 0; j <= endCode - firstCode; ++j)
            {
                ...
                {
                    LOG.warning("Format 12 cmap contains an invalid glyph index");
                    break;
                }

                if (firstCode + j > 0x10FFFF)
                {
                    LOG.warning("Format 12 cmap contains character beyond UCS-4");
                }
    ...
    void processSubtype13(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        for (long i = 0; i < nbGroups; ++i)
        {
            ...
            {
                LOG.warning("Format 13 cmap contains an invalid glyph index");
                break;
            }
            ...
            for (long j = 0; j <= endCode - firstCode; ++j)
            {
                ...
                if (firstCode + j > 0x10FFFF)
                {
                    LOG.warning("Format 13 cmap contains character beyond UCS-4");
                }
    ...
    void processSubtype14(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        LOG.warning("Format 14 cmap table is not supported and will be ignored");
    }
    ...
    void processSubtype4(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        {
            LOG.warning("cmap format 4 subtable is empty");
            return;
        }
        buildGlyphIdToCharacterCodeLookup(maxGlyphId);
    }
        ...
        for (int i = 0; i <= maxSubHeaderIndex; ++i)
        {
            ...
            for (int j = 0; j < entryCount; ++j)
            {
                ...
                if (p >= numGlyphs)
                {
                    ...
                    {
                        LOG.warning(MessageFormat.format("glyphId {0} for charcode {1} ignored, numGlyphs is {2}", p, charCode,
                                numGlyphs));
                        logged.add(p);
                        if (logged.size() > 10)
                        {
                            LOG.warning("too many bad glyphIds, more won't be reported for this table");
