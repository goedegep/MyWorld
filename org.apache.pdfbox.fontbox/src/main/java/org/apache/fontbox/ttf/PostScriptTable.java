...
public class PostScriptTable extends TTFTable
{
    ...
    void read(TrueTypeFont ttf, TTFDataStream data) throws IOException
    {
        ...
        {
            LOG.warning(MessageFormat.format("No PostScript name data is provided for the font {0}", ttf.getName()));
        }
                ...
                for (int i = 0; i < nameArray.length; i++)
                {
                    ...
                    catch (IOException ex)
                    {
                        ...
                        LOG.warning(MessageFormat.format(
                                "Error reading names in PostScript table at entry {0} of {1}, setting remaining entries to .notdef",
        ...
        else if (Float.compare(formatType, 2.5f) == 0)
        {
            ...
            for (int i = 0; i < glyphNames.length; i++)
            {
                ...
                {
                    LOG.info(MessageFormat.format("incorrect glyph name index {0}, valid numbers 0..{1}",
        ...
        else if (Float.compare(formatType, 3.0f) == 0)
        {
            ...
            LOG.info(MessageFormat.format("No PostScript name information is provided for the font {0}", ttf.getName()));
        }
        initialized = true;
    }
