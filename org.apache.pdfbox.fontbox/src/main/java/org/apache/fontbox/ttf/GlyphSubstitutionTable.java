...
public class GlyphSubstitutionTable extends TTFTable
{
    ...
    void read(TrueTypeFont ttf, TTFDataStream data) throws IOException
    {
        ...
        else
        {
            ...
            LOG.warning("lookupListOffset is 0, LookupListTable is considered empty");
        ...
        for (FeatureRecord rec : featureListTable.getFeatureRecords())
        {
            ...
            for (int i = 0; i < indices.length; ++i)
            {
                ...
                {
                    LOG.info(MessageFormat.format("Type {0} GSUB feature '{1}' at index {2} unavailable",
    ...
    private ScriptTable readScriptTable(TTFDataStream data, long offset) throws IOException
    {
        ...
        for (int i = 0; i < langSysCount; i++)
        {
            ...
            if (i > 0 && langSysTags[i].compareTo(langSysTags[i-1]) <= 0)
            {
                ...
                LOG.severe(MessageFormat.format("LangSysRecords not alphabetically sorted by LangSys tag: {0} <= {1}",
            ...
            if (i > 0 && featureTags[i].compareTo(featureTags[i-1]) < 0)
            {
                ...
                if (IS_4_CHAR_WORD.test(featureTags[i]) && IS_4_CHAR_WORD.test(featureTags[i - 1]))
                {
                    ...
                    LOG.info(MessageFormat.format(
                            "FeatureRecord array not alphabetically sorted by FeatureTag: {0} < {1}",
                            featureTags[i], featureTags[i - 1]));
                }
                else
                {
                    LOG.warning(MessageFormat.format("FeatureRecord array not alphabetically sorted by FeatureTag: {0} < {1}",
                            featureTags[i], featureTags[i - 1]));
    ...
    private LookupListTable readLookupList(TTFDataStream data, long offset) throws IOException
    {
        ...
        for (int i = 0; i < lookupCount; i++)
        {
            ...
            if (lookups[i] == 0)
            {
                LOG.severe(MessageFormat.format("lookups[{0}] is 0 at offset {1}", i, data.getCurrentPosition() - 2));
            }
            else if (offset + lookups[i] > data.getOriginalDataSize())
            {
                LOG.severe(MessageFormat.format("{0} > {1}", offset + lookups[i], data.getOriginalDataSize()));
            }
        }
    ...
    private LookupSubTable readLookupSubtable(TTFDataStream data, long offset, int lookupType) throws IOException
    {
        switch (lookupType)
        {
            ...
            default:
                ...
                LOG.info(MessageFormat.format("Type {0} GSUB lookup table is not supported and will be ignored",
                            lookupType));
                return null;
    ...
    private LookupTable readLookupTable(TTFDataStream data, long offset) throws IOException
    {
        ...
        for (int i = 0; i < subTableCount; i++)
        {
            ...
            {
                LOG.severe(MessageFormat.format("subTableOffsets[{0}] is 0 at offset {1}", i,
                        data.getCurrentPosition() - 2));
            }
            else if (offset + subTableOffsets[i] > data.getOriginalDataSize())
            {
                LOG.severe(MessageFormat.format("{0} > {1}", offset + subTableOffsets[i], data.getOriginalDataSize()));
            }
        }
            ...
            for (int i = 0; i < subTableCount; i++)
            {
                ...
                if (substFormat != 1)
                {
                    LOG.severe(MessageFormat.format(
                            "The expected SubstFormat for ExtensionSubstFormat1 subtable is {0} but should be 1 at offset {1}",
                ...
                if (lookupType != 7 && lookupType != extensionLookupType)
                {
                    ...
                    LOG.severe(MessageFormat.format("extensionLookupType changed from {0} to {1} at offset {2}",
        ...
        default:
            // Other lookup types are not supported
            LOG.info(MessageFormat.format("Type {0} GSUB lookup table is not supported and will be ignored", lookupType));
        }
    ...
    private LookupSubTable readSingleLookupSubTable(TTFDataStream data, long offset) throws IOException
    {
        ...
        default:
            LOG.warning(MessageFormat.format("Unknown substFormat: {}", substFormat));
            return null;
        }
    }
    ...
    private int applyFeature(FeatureRecord featureRecord, int gid)
    {
        ...
        for (int lookupListIndex : featureRecord.getFeatureTable().getLookupListIndices())
        {
            ...
            {
                LOG.info(MessageFormat.format(
                        "Skipping GSUB feature '{0}' because it requires unsupported lookup table type {1}",
    ...
    public int getUnsubstitution(int sgid)
    {
        ...
        if (gid == null)
        {
            LOG.warning(MessageFormat.format("Trying to un-substitute a never-before-seen gid: {}", sgid));
            return sgid;
        }
        return gid;
    }
