...
public class GlyfCompositeDescript extends GlyfDescript
{
    ...
    public void resolve()
    {
        ...
        if (beingResolved)
        {
            LOG.severe("Circular reference in GlyfCompositeDesc");
            return;
        }
        beingResolved = true;
    ...
    public int getPointCount()
    {
        if (!resolved)
        {
            LOG.severe("getPointCount called on unresolved GlyfCompositeDescript");
        }
        if (pointCount < 0)
        {
            ...
            if (gd == null)
            {
                LOG.severe(MessageFormat.format("GlyphDescription for index {0} is null, returning 0", c.getGlyphIndex()));
                pointCount = 0;
            }
            else
            {
    ...
    public int getContourCount()
    {
        if (!resolved)
        {
            LOG.severe("getContourCount called on unresolved GlyfCompositeDescript");
        }
        if (contourCount < 0)
        {
            GlyfCompositeComp c = components.get(components.size() - 1);
            ...
            if (gd == null)
            {
                LOG.severe(MessageFormat.format("missing glyph description for index {0}", c.getGlyphIndex()));
                contourCount = 0;
            }
            else
            {
