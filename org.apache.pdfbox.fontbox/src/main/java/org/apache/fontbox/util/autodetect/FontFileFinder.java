    ...
    private void walk(File directory, List<URI> results)
    {
        ...
        for (File file : filelist)
        {
            if (file.isDirectory())
            {
                ...
                if (file.isHidden())
                {
                    LOG.info(MessageFormat.format("skip hidden directory {0}", file));
                    continue;
                }
                walk(file, results);
            }
            ...
            {
                LOG.info(MessageFormat.format("checkFontfile check {0}", file));
                if (checkFontfile(file))
                {
                    LOG.info(MessageFormat.format("checkFontfile found {0}", file));
