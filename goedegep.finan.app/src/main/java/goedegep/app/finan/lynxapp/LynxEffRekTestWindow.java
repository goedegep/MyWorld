...
public class LynxEffRekTestWindow extends AppFrame {
  private static final String         WINDOW_TITLE = "Lynx Effectenrekening Test Window";
  ...
  //Construct the frame
  public LynxEffRekTestWindow(Customization customization, FinanBank finanBank, LynxEffRek effectenRekening) {
...
//    statusPanel.setBackground(getLook().getBackgroundColor());
    ...
    // Transactions Table    
    this.getContentPane().add(new TransactionTable(this, effectenRekening), BorderLayout.CENTER);
...
//    textFieldWinstDifference.setText(cf.format(balanceDifference.add(valueDifference).subtract(nettoStortingDifference)));
  ...
  void showEffectenRekeningWindow() {
    ...
    LynxEffRekWindow effectenrekeningWindow = new LynxEffRekWindow(getCustomization(), finanBank, effectenRekening, null, lastHandledDate);
