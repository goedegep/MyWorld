...
public class ShareRatesTable extends AppGenAbstractTable {
  public ShareRatesTable(AppFrame owner) {
    super(owner, 500, 400);

    JTable table = getTable();
    table.setModel(new ShareRatesModel());
  ...
  public void setParameters(Fund fund, Share share, RateType rateType) {
    ((ShareRatesModel)getTable().getModel()).setParameters(fund, share, rateType);
    getTable().revalidate();
    getTable().repaint();
  }
}