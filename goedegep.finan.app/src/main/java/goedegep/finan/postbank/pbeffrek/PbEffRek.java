...
public class PbEffRek extends EffRek {
  ...
  public PbEffRek(StockDepot parentDepot) {
    super(parentDepot, PbBasic.effectenRekeningString, PgCurrency.GUILDER);
  }
  ...
  public void dumpData(TextWriter textWriter) throws IOException {
    textWriter.write("POSTBANK EFFECTENREKENING");
    textWriter.newLine();
  ...
  private void dumpTransactions(BufferedWriter out) throws IOException {
    out.write("POSTBANK EFFECTENREKENING TRANSACTIONS");
    out.newLine();
  ...
  private void dumpKwartaalOverzichten(BufferedWriter out) throws IOException {
    out.write("POSTBANK EFFECTENREKENING KWARTAALOVERZICHTEN");
    out.newLine();
...
//    }
  ...
  private void dumpKwartaalOverzicht(StockDepotPeriodicReport<Quarter> quarterReport, BufferedWriter out) throws IOException {
    Quarter quarter = quarterReport.getPeriod();
    out.write("POSTBANK EFFECTENREKENING KWARTAALOVERZICHT " + QF.format(quarter));
    out.newLine();
    ...
    if (totaalWaarde != null) {
      out.write(CF_LARGE.format(totaalWaarde));
    }      
    out.newLine();
  }

  private void dumpFiscaleJaarOverzichten(BufferedWriter out) throws IOException {
    out.write("POSTBANK EFFECTENREKENING FISCALE JAAROVERZICHTEN");
    out.newLine();
...
//    }
  ...
  private void dumpFiscaalJaarOverzicht(StockDepotPeriodicReport<Integer> taxReport, BufferedWriter out) throws IOException {
    int year = taxReport.getPeriod();
    out.write("POSTBANK EFFECTENREKENING FISCAAL JAAROVERZICHT " + year);
    out.newLine();
    ...
    for (DividendOntvangst dividend: dividends) {
      ...
      out.write(CF_LARGE.format(dividendBelasting));
      out.write('\t');
      ...
      out.write(CF_LARGE.format(provisie));
      out.write('\t');
      ...
      out.write(CF_LARGE.format(nettoBedrag));
      out.write('\t');
    ...
    if (totaalBrutoBedrag != null) {
      out.write(CF_LARGE.format(totaalBrutoBedrag));
    }
    out.write('\t');
    out.write("totaal dividendbelasting: ");
    if (totaalDividendBelasting != null) {
      out.write(CF_LARGE.format(totaalDividendBelasting));
    }      
    out.write('\t');
    out.write("totaal provisie: ");
    if (totaalProvisie != null) {
      out.write(CF_LARGE.format(totaalProvisie));
    }      
    out.write('\t');
    out.write("totaal netto bedrag: ");
    if (totaalNettoBedrag != null) {
      out.write(CF_LARGE.format(totaalNettoBedrag));
    }      
    out.newLine();
  }
  
  private void dumpVModelStatussen(BufferedWriter out) throws IOException {
    out.write("POSTBANK EFFECTENREKENING VMODEL STATUSSEN");
    out.newLine();
  ...
  private void dumpVModelStatus(BufferedWriter out, StockPosition stockPosition) throws IOException {
    ...
    out.write("POSTBANK EFFECTENREKENING VMODEL STATUS: ");    
    ...
    if (status == null) {
      out.write("Geen VModel status berekening mogelijk i.v.m. met onvolledige of foute transacties.");
      return;
    }
