package goedegep.util.bytearray;


/**
 * This is a utility class, with utility methods related to bytes and byte arrays.
 */
public final class ByteArrayUtils {
  private ByteArrayUtils() {
  }

  public static int compareBytes(byte[] first, int firstStartIndex,
                     byte[] second, int secondStartIndex, int length) {
    int firstIndex = firstStartIndex;
    int secondIndex = secondStartIndex;

    for (int i = 0; i < length; i++) {
      if (first[firstIndex] == second[secondIndex]) {
        firstIndex++;
        secondIndex++;
        continue;
      } else if (first[firstIndex] > second[secondIndex]) {
        return 1;
      } else {
        return -1;
      }
    }

    return 0;
  }

  
  /**
   * Convert a byte array to a human readable string.
   */
  public static String toHex(byte[] bytes) {
    StringBuilder buf= new StringBuilder("[ ");
      for (int i = 0; i < bytes.length; i++) {
          buf.append(getHex(bytes[i])).append(' ');
      }
      buf.append(']');
      return buf.toString();
  }
  
  public static String toChars(byte[] bytes) {
    StringBuilder buf = new StringBuilder("[ ");
    
    for (int i = 0; i < bytes.length; i++) {
      char c = (char) bytes[i];
      buf.append(c).append(' ');
    }
    
    buf.append(']');
    
    return buf.toString();
  }

  /**
   * Convert a byte to a printable format.
   * @param b
   * @return
   */
  public static String getHex(byte b) {
      char char0, char1;
      byte byte1= (byte) (0x0f & (b >> 4));
      byte byte2= (byte) (0x0f & b);
      char0 = (char) (byte1 > 9 ? 'a' + byte1 - 10 : '0' + byte1);
      char1 = (char) (byte2 > 9 ? 'a' + byte2 - 10 : '0' + byte2);
      return "" + char0 + char1;
  }
  
  /**
   * Convert a byte array to a binary string representation.
   * <p>
   * Each byte is printed on a separate line.
   * Each byte is printed as a sequence of 8 ones and/or zeros.
   * 
   * @param data the byte array to be converted.
   * @return a binary string representation of the {@code data}
   */
  public static String arrayToBinaryString(byte[] data) {
    StringBuffer buf = new StringBuffer();
    for (int i = 0; i < data.length; i++) {
      buf.append(byteToBinaryString(data[i]) + System.getProperty("line.separator"));
    }
    return buf.toString();
  }
  
  private static String byteToBinaryString(byte value) {
    StringBuffer buf = new StringBuffer();
    for (int i = 7; i >= 0; i--) {
      if ((value & ((byte) 0x01 << i)) == 0) {
        buf.append('0');
      } else {
        buf.append('1');
      }
    }
    return buf.toString();
    
  }
}
