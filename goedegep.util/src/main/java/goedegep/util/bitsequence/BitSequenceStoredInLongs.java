 ...
 * 
 */
public class BitSequenceStoredInLongs {
  private static final Logger LOGGER = Logger.getLogger(BitSequenceStoredInLongs.class.getName());
  ...
  private final static int HEIGHEST_BIT_INDEX_IN_WORD = BITS_PER_WORD - 1;


  /**
   * The array of longs to store the "bits".
   */
  private long[] words;
   ...
   */
  public BitSequenceStoredInLongs() {
    initWords(BITS_PER_WORD);
  }
  
  /**
   * Constructor where the buffer is filled from a byte array.
   *
   * @param data the byte array to fill the BitSequence.
   */
  public BitSequenceStoredInLongs(byte[] data) {
    int nrOfWords = (data.length + 7) / 8;
    words = new long[nrOfWords];
    
    for (int wordIndex = 0; wordIndex < nrOfWords; wordIndex++) {
      long word = 0;
      for (int subIndex = 0; subIndex < 8; subIndex++) {
        int byteIndex = 8 * wordIndex + subIndex;
        long byteValue = 0;
        
        if (byteIndex < data.length) {
          byteValue = data[byteIndex];
          byteValue &= 0xff;
          byteValue = byteValue << (7 - subIndex) * 8;
        }
        word |= byteValue;
        LOGGER.fine("byteValue=0x" + Long.toHexString(byteValue) + ", word=0x" + Long.toHexString(word));
      }
  ...
  /**
   * Create initial <code>words</code> array.
   * 
   * @param nrOfBits initial number of bits that will fit in the array.
   */
  private void initWords(int nrOfBits) {
      words = new long[wordIndex(nrOfBits-1) + 1];
  }  
  ...
  public void set(int bitIndex) {
      ...
      int bitInWordIndex = bitInWordIndex(bitIndex);

      words[wordIndex] |= (1L << bitInWordIndex);
  ...
  public boolean get(int bitIndex) {
      ...
      int wordIndex = wordIndex(bitIndex);
      int bitInWordIndex = bitInWordIndex(bitIndex);
      return (words[wordIndex] & (1L << bitInWordIndex)) != 0;
  }
  ...
  private static int bitInWordIndex(int bitIndex) {
    return HEIGHEST_BIT_INDEX_IN_WORD - (bitIndex % BITS_PER_WORD);
  }
  ...
  public void addByteArray(byte[] bytes) {
    LOGGER.info("Before adding byte array: length =" + length);
    for (byte byteValue: bytes) {
      addByte(byteValue);
    }
    LOGGER.info("After adding byte array: length =" + length);
  }
   ...
   */
  public void addBitSequence(BitSequenceStoredInLongs bitSequence) {
  ...
  public void addFixedSizeInteger(int value, int nrOfBits) {
    LOGGER.info("=> value=" + value + ", nrOfBits=" + nrOfBits);
  ...
  public byte[] toByteArray() {
    LOGGER.info("=> Length of byte array=" + length);
    int bytesInUse = (length + 7) / 8;
    LOGGER.fine("bytesInUse=" + bytesInUse);
    int wordsInUse = (length + HEIGHEST_BIT_INDEX_IN_WORD) / BITS_PER_WORD;
    LOGGER.fine("wordsInUse=" + wordsInUse);
    
    if (bytesInUse == 0) {
      return new byte[0];
    }
    
    byte[] bytes = new byte[bytesInUse];
    
    for (int wordIndex = 0; wordIndex < wordsInUse; wordIndex++) {
      LOGGER.fine("wordIndex=" + wordIndex);
      long word = words[wordIndex];
      LOGGER.fine("word=0x" + Long.toHexString(word));
      for (int subIndex = 7; subIndex >= 0; subIndex--) {
        int byteIndex = 8 * wordIndex + subIndex;
        long byteValue = word & 0xff;
